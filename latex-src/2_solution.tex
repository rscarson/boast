...
To address the challenges outlined in Section 1, we introduce BOAST (Bayesian Outlier-Aware Sequential Testing), a method for efficiently estimating the minimal number of test iterations required to achieve a desired confidence level of detecting failure in code consuming numerical data, particularly while testing against synthetic data with known noise characteristics.

BOAST combines a modified Bayesian update procedure with a frequentist stopping criterion, and incorporates a damping factor to control conservatism in the initial estimate of failure probability. The method is highly tunable, has no systemic bias towards false positives or false negatives, and is robust to mis-specification of its input parameters, making it suitable for a wide range of use cases and risk tolerances.

The output of the test includes both a pass/fail result, and a confidence interval for the true failure rate among data sets, allowing users to make informed decisions about the reliability of their code and the potential risks associated with it. The method is designed to be computationally efficient, making it suitable for use in CI pipelines and other time-sensitive contexts.
...
\begin{figure}[ht!]
    ...
    \caption{Flowchart of the BOAST testing procedure.}
    \label{fig:boast_flowchart}
\end{figure}

The BOAST method consists of three main phases, depicted in Figure \ref{fig:boast_flowchart}: the estimation phase, the iterative testing phase, and the output phase. 
...
\begin{description}[labelwidth=2em, leftmargin=!]
    ...
    \item[$p$] The likelihood of any individual point being an outlier, chosen conservatively (underestimation of the true risk).
    \item[$q$] The desired confidence that enough iterations have been performed to have observed a failure, if one was present.
...
\begin{description}[labelwidth=5em, leftmargin=!]
    \item[$C$] A damping constant reflecting the diminishing impact of additional data points on the likelihood of failure.
    \item[$p_s$] A strength parameter for the Bayesian prior, reflecting the confidence in the initial estimate of the failure probability.
\end{description}

Derivations for suggested values of the above constants are included in Section \ref{sec:derivations}.

$q$ represents the target confidence $\gamma$ of observing a failing set within the sample population ($\Pr[E] \ge q$). It should be selected to be a reasonably conservative value, as defined in Section \ref{sec:derivations}, with higher values indicating a lower tolerance for risk, and therefore a desire for more iterations to be performed.

$p$ should be selected to be a conservative underestimate of the true point-wise probability of outliers, to ensure that the algorithm does not underestimate the required number of iterations. It can be thought of as the user's prior belief about the likelihood of any individual value in the input data being unusual or extreme enough to contribute to a failure in the code under test. 

$\operatorname{pass\_ratio}$ should be selected based on the user's tolerance for test failures, for example in the case of intermittent network failures, or the effects of non-determinism in the generated data sets. This parameter should be optional, defaulting to $1.0$ (i.e., no failures observed in the sample), and can be used to allow for a certain level of observed failure in the sample while still considering the test a success, which may be desirable in cases where some level of failure is expected or acceptable.

$C$, the damping constant, controls the conservatism of the initial estimate for $p_{\text{fail}}$. Our recommended value is derived in Section \ref{sec:derivation_c} using a formulation based on the minimum number of iterations required for a test result to be considered statistically meaningful, and a value of confidence that is considered the minimum reasonably conservative value. Our recommended value for $C$ is $1.864$, which corresponds to a minimum number of iterations of 3 at a confidence level of 80\%.

$p_s$, the strength of the Bayesian prior, is derived in Section \ref{sec:derivation_ps} using a method that identifies a Pareto-optimal tradeoff between the accuracy of the test and the computational cost of the test procedure. Our recommended value for $p_s$ is $3.35 \cdot 10^{6}$.

The sections below describe the 3-phase approach for the BOAST method.
...
$p'$ is used to model the saturation of the effect of individual outliers on set-wise failure. It represents the inverse relationship $\frac{1}{np}$ between number of expected outliers, and their effects on iterations $k$. For more on the derivation of this form, and the recommended value for $C = 1.864 $, see Section \ref{sec:derivation_c}. We call the ratio applied to $p$ the "damping fraction", since it serves to dampen the effect of increasing $n$ on $p_{\text{fail}}$.

For small $n$ the probability of failure is roughly linear in $n$, consistent with a binomial model where $p_{\text{fail}} \approx 1 - (1 - p)^n$. As $n$ becomes large, the damping fraction saturates, and the effective failure probability approaches a limiting value $p_{\text{fail}} = 1 - e^{-1/C}$. In this regime, the number of failures per iteration can be approximated by a Poisson distribution with $\lambda = \frac{1}{C}$, reflecting the diminishing returns of increasing data set size on the likelihood of failure $F(X)$ due to the presence of outliers in the set.
...
Thus, if $iterations \ge k$, we can be confident that we would have observed at least one failure if the true failure probability is equal to or greater than our current estimate, and thus we can stop the test and move on to the output phase.

\subsubsubsection{Timeout Criteria}
To prevent unreasonable runtimes in the case of extremely low failure probabilities in the absence of observed failure, we recommend implementing a timeout mechanism. If the total elapsed time for the testing process exceeds a predetermined threshold, the test should terminate and report failure due to insufficient evidence to support a claim.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Increment $\beta$ by $passes\_since\_failure$, and reset $passes\_since\_failure = 0$
...
In order to provide a robust estimate of the true underlying failure rate amongst data sets from the sample collected by BOAST, we construct a 95\% Wilson score confidence interval \cite{wilson1927}. This method is chosen for its superior performance compared to other methods such as the normal approximation interval, particularly in cases of small sample sizes or extreme proportions, which are common in our testing context.

The interval bounds are calculated as follows:
\begin{equation}
p_{\text{fail, lower/upper}} = \frac{1}{1 + \frac{z^2}{k}} \left[ \hat{p} + \frac{z^2}{2k} \pm z \sqrt{\frac{\hat{p}(1-\hat{p})}{k} + \frac{z^2}{4k^2}} \right]
\end{equation}

where:
\begin{itemize}
    \item $\hat{p} = 1 - \text{pass\_ratio}$ is the observed failure proportion.
    \item $k$ is the total number of iterations (sample size).
    \item $z$ is the $1 - \frac{\alpha}{2}$ quantile of the standard normal distribution (e.g., $1.96$ for 95\% confidence).
\end{itemize}

This method cleanly seperates the Beta function used to estimate the required size of the sample $k$ from the analysis of the observed sample, which allows for a more robust estimate of the true underlying failure rate among data sets given the purposely conservative nature of the initial estimate for $p_{\text{fail}}$.

The outputs for the test are then:
\begin{itemize}
    \item $\text{test result}$ The test result
    \item $r_\text{obs}$, the observed pass ratio
    \item The calculated 95th percentile bounds on $p_{\text{fail, true}}$
\end{itemize}

These can be communicated succinctly as:
``$[100 \cdot r_\text{obs}]\%$ of tests passed. With 95\% confidence, the true failure rate is between $[100 \cdot p_{\text{fail, lower}}]\%$ and $[100 \cdot p_{\text{fail, upper}}]\%$.''
...
The proposed method has several limitations and assumptions worth noting: \begin{itemize}
    ...
    \item The method assumes that the probability of outliers is independent and normally distributed across the data set, which may not hold true in all cases.
    \item Very small $n$ ($< 100$) require a very large number of iterations to converge; this is an expected by-product of the model used, and we therefore recommend $n >= 100$
\end{itemize}
...
We will define that practical range for $k_{\text{floor}}$ to be between 3 and 10 iterations, and then we will define the lowest confidence $q$ we consider reasonably conservative to be $0.8$.

The practical lower bound for $k_{\text{floor}}$ is defined by the lowest number of iterations that could be considered valid in a test such as the Rule of Three, where $k < 3$ would produce a confidence level above 100\% for any observed pass ratio, and thus would not be useful for any real-world use case.

We can analyze the effect of $C$ effect on $k_{\text{floor}}$ as n approaches infinity:
\begin{align}
    p' &= \frac{p}{1 + C n p} \\
    p_{\text{fail}} &= 1 - (1 - p')^n
\end{align}
...
Since $- \ln(1 - q)$ represents the cumulative hazard function, we can interpret $C$ in units of $\operatorname{iterations} / nat$, or the average number of iterations required to resolve $1 nat$ of uncertainty.

If we set $k_{\text{floor}} = 3$ as a reasonable minimum number of iterations that could be considered useful, given a lower-bound on the value for $q$ we consider reasonably conservative of $q = 0.8$, we find:
\[
C = \frac{k_{\text{floor}}}{- \ln(1 - q)} 
\quad \Rightarrow \quad 
...
\]
we can evaluate the sensitivity of $k_{\text{floor}}$ to small variations in $C$ for a fixed confidence level of $q = 0.8$.

Using the derived nominal value $C = 1.864$:
\begin{align*}
k_{\text{floor}}
    &= -(1.864)\ln(1 - 0.8)
    \approx 2.99999
...
Taking into account the effect of the ceiling operation in the calculation of $k_{\text{initial}}$, variations in $C$ of approximately $\pm 0.5$ around the nominal value of $1.864$ produce a change of approximately $\pm 0.804$ in $k_{\text{floor}}$, indicating that the formulation is reasonably robust to small perturbations in $C$.

We will therefore recommend $C = 1.864$ as the nominal value for the damping constant. Since $1$ nat represents $\approx 1.44$ bits of information, this corresponds to an average of $1.292$ iterations per bit of information gained about the failure probability.
...
We recommend selecting a new value if the scale of compute available for regular testing is significantly above what is standard. $k_{\text{floor}}$ should then be selected to reflect the smallest number of iterations that would have statistical meaning in context. $q$ should be selected to reflect the confidence in that floor, that is the resistance to growing $k_{\text{floor}}$ as $q$ increases.
...
These parameters were chosen since they are calculated directly from $n$ and $p$, and thus allow us to infer the effect of those parameters on bias.
...
Our hypothesis was that the delta between the expected level of failure $p_{\text{fail}}$ and the acceptable level of failure $1 - r_\text{pass}$ would be a good predictor of bias.

However, as can be seen in Figure \ref{fig:ps_experiment1}, neither parameter had a measurable effect on bias - which appeared to be randomly distributed around 0 with a constant error regardless of the values of $p_{\text{fail}}$ and $\operatorname{pass\_ratio}$. Since $p_{\text{fail}}$ encodes for $k$, and contains both $n$ and $p$, we can infer that none of $n$, $p$, $p_{\text{fail}}$, or $k$ have a significant effect on bias, which appears to be independent of any inputs into the model given an unknown underlying failure rate.
...
Following the experiment in Section \ref{sec:derivation_ps}, we fit the relationship between the performed fraction of maximum iterations and $p_s$ to a pair of Hill equations \cite{Gesztelyi2012} of the form:
\begin{align}
