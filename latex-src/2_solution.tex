...
\begin{description}[labelwidth=5em, leftmargin=!]
    \item[$C$] A damping constant reflecting the diminishing impact of additional data points on the likelihood of failure due to extreme outliers
...
$p$ should be selected to be a very conservative underestimate of the true point-wise probability of extreme outliers, to ensure that the algorithm does not underestimate the required number of iterations. It can be thought of as the user's prior belief about the likelihood of any individual value in the input data being unusual or extreme enough to contribute to a failure in the code under test. 

$q$ should be selected based on the user's tolerance for risk, with higher values indicating a lower tolerance for risk, and therefore a desire for more iterations to be performed.

$\operatorname{pass\_ratio}$ should be selected based on the user's tolerance for test failures, for example in the case of intermittent network failures, or the effects of non-determinism in the generated data sets. 

$C$, the damping constant, controls the conservatism of the initial estimate for $p_{\text{fail}}$. Its value is derived in Section \ref{sec:derivation_c} using a formulation based on the minimum number of iterations required for a test result to be considered statistically meaningful. Our recommended value for $C$ is $1.864$, which corresponds to a minimum number of iterations of 3 at a confidence level of 80\%.
