...
The practical range for $k_{\text{floor}}$ is defined between the lowest number of iterations that could be considered valid in a test such as the Rule of Three, where $k < 3$ would produce a confidence level above 100\% for any observed pass ratio, and thus would not be useful for any real-world use case.
...
Taking into account the effect of ceil in the true calculation of $k_{\text{initial}}$, we can see that variations in $C$ of approximately $\pm 0.5$ around the nominal value of $1.864$ produce a difference of approximately $\pm 1$ in the final value, showing that this method is reasonably robust to small variations in $C$.
