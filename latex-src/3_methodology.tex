...
\label{sec:p_fail_experiment3}
    ...
    \begin{figure}[ht!]
        ...
        \caption{Graph showing the mean and standard deviation of lower-bound confidence intervals on the sample of iterations observed by BIOD from a simulation of 10,000 trials, compared to the simulated true $p_\text{fail}$ for various dataset sizes $n$, and for various values of point-wise $p$. Green was deliberately underestimated, blue was exact, and red was deliberately overestimated. Markers indicate the number of iterations $k$ performed by BIOD at each point.}
        \label{fig:p_fail_experiment3}
    \end{figure}

    \paragraph{Objective.}
    To verify that the calculated $p_\text{fail}$ from BIOD provides accurate confidence intervals that encompass the observed $p_\text{fail}$ across a range of dataset sizes $n$ and point-wise $p$ values.
    ...
    \subsubsection{Setup.}
        ...
        \begin{itemize}
            ...
            \item Run BIOD with the same results to calculate confidence intervals on $p_\text{fail}$ for each value of point-wise $p$:
            \begin{itemize}
    ...
    \subsubsection{Hypothesis.}
        ...
        The BIOD-calculated confidence intervals for $p_\text{fail}$ are expected to encompass the observed $p_\text{fail}$ for accurate and underestimated point-wise $p$ values, and to potentially miss for overestimated point-wise $p$ values, across all tested set sizes $n$.

    \subsubsection{Results.}

        As shown in Figure~\ref{fig:p_fail_experiment3}, the calculated confidence intervals for $p_\text{fail}$ from BIOD successfully encompass the observed $p_\text{fail}$ for both all tested set sizes $n$ even for badly specified point-wise $p$ values.
    ...
    These results show that BIOD is robust to badly specified point-wise $p$ parameters, but gains significant accuracy when point-wise $p$ is well specified or conservative (i.e., underestimated).

    An overly conservative (underestimated) point-wise $p$ leads to tighter confidence intervals, due to the larger number of iterations BIOD performs in this case, improving the accuracy of the $p_\text{fail}$ estimate.

    An overly aggressive (overestimated) point-wise $p$ leads to wider confidence intervals, especially for small $n$, due to the smaller number of iterations BIOD performs in this case, reducing the accuracy of the $p_\text{fail}$ estimate. However, as $n$ increases, the effect of point-wise $p$ mis-specification diminishes, and the confidence intervals converge towards those obtained with exact point-wise $p$.

    \subsubsection{Summary / Recommendation.}

    The BIOD method provides reliable confidence intervals for $p_\text{fail}$ estimates, particularly when point-wise $p$ is well specified or underestimated, but is still useful when point-wise $p$ is overestimated.
...
\subsection{Experiment 7: Prior Strength Derivation} \label{sec:ps_experiment4}
    ...
    \begin{figure}[ht!]
        ...
        \caption{Graphs the true $p_\text{fail}$ values against the reported confidence intervals from BIOD for various $n$ (left) and the width of the confidence intervals against $n$ (right).}
        \label{fig:estimate_experiment1}
    \end{figure}

    \paragraph{Objective.}
    To evaluate the quality of the final $p_\text{fail}$ estimate provided by BIOD, by comparing the true $p_\text{fail}$ values from simulations against the confidence intervals reported by BIOD across a range of dataset sizes $n$.
    ...
    For each $n$ ranging from $10^0$ to $10^5$, we simulate $100 \cdot k$ datasets (where $k$ is BIOD's sample size) to establish a "ground truth" simulated failure rate. We then run BIOD with a 95\% confidence target and compare its reported Wilson interval bounds against this ground truth.

    \subsubsection{Definition of Metrics.}

    \begin{itemize}
        \item $p_\text{fail, true}$: The true failure probability obtained from the simulations for each dataset size $n$.
        \item $p_\text{fail, biod, min}$: The lower bound of the confidence interval for $p_\text{fail}$ reported by BIOD.
        \item $p_\text{fail, biod, max}$: The upper bound of the confidence interval for $p_\text{fail}$ reported by BIOD.
        \item Interval width: The width of the confidence interval reported by BIOD, calculated as $p_\text{fail, biod, max} - p_\text{fail, biod, min}$.
    \end{itemize}

    \subsubsection{Hypothesis.}

    We hypothesize that the true failure probability $p_\text{fail, true}$ will consistently fall within the confidence intervals reported by BIOD across all tested dataset sizes $n$, and that the width of these confidence intervals will increase as $n$ increases, reflecting a small initial estimated $k$.

    \subsubsection{Results.}

    We find that the true failure probability $p_\text{fail, true}$ indeed falls within the confidence intervals reported by BIOD for all tested dataset sizes $n$, confirming the reliability of BIOD's estimates.

    While the width of the confidence intervals increases with $n$, it appears to stabilize between $n = 10^3$ and $n = 10^4$, suggesting that BIOD's sample size is sufficient to provide accurate estimates without excessive widening of the confidence intervals at larger dataset sizes.

    \subsubsection{Analysis / Interpretation.}

    The results confirm that BIOD provides accurate confidence intervals for $p_\text{fail}$ estimates across a wide range of dataset sizes, with the true failure probability consistently falling within these intervals.

    \subsubsection{Summary / Recommendation.}

    BIOD's final $p_\text{fail}$ estimates are reliable and can be confidently used for decision-making, with the understanding that confidence interval widths may increase with larger dataset sizes but remain informative.

    The effect can be mitigated with more conservative point-wise $p$ values, which would lead to larger initial $k$ estimates and thus more iterations performed by BIOD, improving the accuracy of the $p_\text{fail}$ estimate and reducing confidence interval widths at larger dataset sizes.